...
export class S3BucketStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Get name from context:  -c bucketName=..., if so -c user=...
    const rawName =
      (this.node.tryGetContext('bucketName') as string | undefined) ??
      (this.node.tryGetContext('user') as string | undefined);

    // Check variable
    if (!rawName || rawName.trim() === '') {
      throw new Error('Please use this "-c bucketName=<name>" or "-c user=<name>"');
    }
    
    const bucketName = `bucket-${rawName}`;

    // Create S3 with  public read access
    const bucket = new s3.Bucket(this, 'MyPublicBucket', {
      removalPolicy: RemovalPolicy.DESTROY,
      bucketName,                             // <-- name as context
